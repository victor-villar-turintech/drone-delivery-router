...
logger = logging.getLogger("drone_delivery.dispatch")
...
async def dispatch_order(order_id: int, db: AsyncSession = Depends(get_db)):
    with benchmark("dispatch_order") as bench:
        order = await db.get(Order, order_id)
        if not order:
            raise HTTPException(status_code=404, detail="Order not found")
    ...
    logger.info(
        "Dispatch complete: order=%d drone=%d distance=%.4f km battery_cost=%.2f%%",
        order.id, drone.id, total_distance, total_battery,
    )
